...
class GPUManager:
    ...
    def initialize(self) -> torch.device:
        ...
        if self.config.available and self.args.enable_auto_gc:
            Logger.info("\nGPU 内存管理:")
            Logger.success("  自动垃圾回收: 已启用")
            Logger.info(f"  检查间隔: {self.args.auto_gc_interval} 秒")
            Logger.info(f"  清理阈值: {self.args.auto_gc_threshold}%")
    ...
    def clear_cache(self, device_id: int = 0) -> bool:
        ...
        try:
            ...
            if freed_mb > 1:
                Logger.info(f"  释放显存: {freed_mb:.2f} MB")
    ...
    def force_gc(self, device_id: int = 0) -> bool:
        ...
        try:
            ...
            if freed_mb > 1:
                Logger.info(f"  回收显存: {freed_mb:.2f} MB")
    ...
    def smart_reclaim(self, threshold_percent: float = 85.0, device_id: int = 0) -> bool:
        ...
        try:
            ...
            if mem_info['used_percent'] >= threshold_percent:
                ...
                Logger.info(f"  总显存: {mem_info['total_mb']:.2f} MB")
                Logger.info(f"  已用: {mem_info['used_mb']:.2f} MB")
                Logger.info(f"  可用: {mem_info['free_mb']:.2f} MB")
    ...
    def start_auto_monitor(self, interval_seconds: int = 30, threshold_percent: float = 85.0):
        ...
        def monitor_loop():
            """监控循环"""
            Logger.info(f"GPU 自动内存监控已启动 (间隔: {interval_seconds}s, 阈值: {threshold_percent}%)")
